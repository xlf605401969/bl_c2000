/**
 * @file bl_flash_8bit.h
 * @brief Flash驱动层接口定义
 *
 * 注意：CM核心为8位寻址，每个地址对应一个字节(8bit)。
 * 本驱动中所有地址和长度参数均以字节为基本单位，匹配CM本地内存模型。
 ...
 * 256字节
 */
#define BL_FLASH_CACHE_PAGE_SIZE 256
 ...
 * @brief 扇区信息结构体
 *
 * 记录每个扇区的起始地址和大小(以字节为单位)
 */
typedef struct
{
    uint32_t start_address; /**< 扇区起始地址(字节地址) */
    uint32_t size;          /**< 扇区大小(以字节为单位) */
...
typedef struct
{
    uint32_t base_addr;                                      /**< 缓存基地址(字节地址) */
    uint8_t data[BL_FLASH_CACHE_PAGE_SIZE];           /**< 缓存数据区(以8位字节为单位) */
 ...
 * @param actual_addr 实际擦除起始地址(字节地址)
 * @param actual_size 实际擦除大小(以字节为单位)
 * @return 操作结果
 */
 ...
 * 5. 处理跨页写入
 *
 * @param addr 写入起始地址(字节地址)
 * @param data 写入数据缓冲区指针
 * @param size 写入数据大小(以字节为单位)
 * @return 操作结果
 */
 ...
 * @brief 获取扇区起始地址
 * @param sector_num 物理扇区号(1-13)
 * @return 扇区起始地址(字节地址)，失败返回0xFFFFFFFF
 */
