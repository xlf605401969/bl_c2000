...
int bl_flash_cm_erase(uint32_t addr, uint32_t size, uint32_t* actual_addr, uint32_t* actual_size)
{
    ...
    uint8_t sector_num = bl_flash_cm_addr_to_sector(addr);
    if (sector_num == 0xFF) {
        return BL_INVALID_PARAM;
    }

    bl_flash_sector_info_t *sector = NULL;
    for (int i = 0; i < sizeof(cm_sectors) / sizeof(cm_sectors[0]); i++) {
        if (cm_sectors[i].sector_num == sector_num) {
            sector = &cm_sectors[i];
            break;
        }
    }
    ...
    Fapi_StatusType status;
    status = Fapi_issueAsyncEraseCommand(
        (uint32_t *)sector->start_address,
        sector->size,
        0,
        0,
        Fapi_FlashBank0);
    ...
    if (actual_addr != NULL) {
        *actual_addr = sector->start_address;
    }

    if (actual_size != NULL) {
        *actual_size = sector->size;
    }
...
int bl_flash_cm_write(uint32_t addr, const uint16_t* data, uint32_t size)
{
    ...
    while (remaining > 0) {
        ...
        status = Fapi_issueProgrammingCommand((uint32_t *)current_addr,
                                               &data[write_offset],
                                               (uint16_t)chunk_size,
                                               NULL,
                                               0,
...
uint8_t bl_flash_cm_addr_to_sector(uint32_t addr)
{
    ...
    for (int i = 0; i < sizeof(cm_sectors) / sizeof(cm_sectors[0]); i++) {
        uint32_t sector_end = cm_sectors[i].start_address + cm_sectors[i].size;
        if (addr >= cm_sectors[i].start_address && 
            addr < sector_end) {
            return cm_sectors[i].sector_num;
        }
    }
...
uint32_t bl_flash_cm_get_sector_start_addr(uint8_t sector_num)
{
    ...
    for (int i = 0; i < sizeof(cm_sectors) / sizeof(cm_sectors[0]); i++) {
        if (cm_sectors[i].sector_num == sector_num) {
            return cm_sectors[i].start_address;
        }
    }
